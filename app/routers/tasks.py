...
async def get_current_user_uid(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=(
                "La sesión ha expirado. Por favor, inicia sesión "
                "nuevamente."
            ),
        )
...
async def create_task(
    ...
    genai_client: Annotated[
        genai.Client,
        Depends(get_genai_client),
    ],
) -> TaskResponse:
    """Crea una tarea, invoca a Gemini para asignar prioridad y persiste
    en Firestore.
    ...
    Flujo:
        ...
        4. Respuesta — retorna el objeto creado con su ID generado por
           Firestore.
    """
    # --- Paso 2: Priorización por Gemini ---
    prompt = (
        "Eres un asistente de gestión de tareas. Analiza la siguiente tarea "
        "y responde ÚNICAMENTE con una de estas tres palabras en minúsculas: "
        "'low', 'medium' o 'high'.\n\n"
    ...
    try:
        ...
        if priority not in _VALID_PRIORITIES:
            logger.warning(
                "Gemini retornó un valor inesperado '%s'. "
                "Se asigna 'medium'.",
                priority,
            )
            priority = "medium"
...
@router.get(
    ...
    summary=(
        "Listar tareas del usuario autenticado, con filtro opcional "
        "por prioridad"
    ),
)
...
@router.post(
    ...
    status_code=status.HTTP_200_OK,
    summary=(
        "Enviar una notificación de prueba al servicio "
